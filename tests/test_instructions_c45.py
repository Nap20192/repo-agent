...
from scanner.adapter.tools import rosters
from scanner.app import agents, instructions
...
def test_review_instruction_names_the_13_checklist_keys_statuses_and_the_fp_gate():
    t = instructions.REVIEW_INSTRUCTION
    assert instructions.REVIEW_CHECKLIST == REVIEW_KEYS and len(REVIEW_KEYS) == 13
    ...
    assert instructions.OPERATING_PRINCIPLES in t


def test_viability_instruction_has_the_routes_and_the_fail_safe():
    t = instructions.VIABILITY_INSTRUCTION
...
def test_factories_build_agents_with_budgets_schemas_and_tools(tmp_path):
    run = FakeRun()
    rv = agents.new_review("m", rosters.review_tools(run, tmp_path), 3)
    vi = agents.new_viability("m", rosters.viability_tools(run, tmp_path), 2)
    co = agents.new_confirm("m", rosters.confirm_tools(run, tmp_path), 5)
    tb = agents.new_triage_batch("m", rosters.triage_tools(run, tmp_path), 4)
    ...
    assert {t.__name__ for t in rv.tools} == {t.__name__ for t in rosters.critic_tools(run, tmp_path)}
