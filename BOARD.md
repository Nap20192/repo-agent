...
| 43 | migrate PipelineV2 to the ADK 2.9 Workflow graph (FunctionNode pre-pass, single_turn stages with output_schema, JoinNode reconciler, dynamic investigator node with parallel workers, terminal reporter); ADR-0001 revisited | architect agent → dev | planned | scanner/app/pipeline_v2.py, graph.py, runner.py, web/fullscan/agent.py, docs/adr/0007 | blueprint reviewed at GATE 1; all pipeline tests green on the Workflow root; adk web renders the graph |
