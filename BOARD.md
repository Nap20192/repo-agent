...
| 42 | direct findings lane: osv/gitleaks/semgrep-ERROR anchors → store.report without LLM (dedup, OSV/EPSS/KEV/fixed-version enrichment, reachability note, secrets redacted); excluded from the queue and the Critic; confidence normalisation in the gate | fork agent | in progress | scanner/app/reconcile.py (split_direct), scanner/app/pipeline_v2.py, scanner/adapter/knowledge.py, scanner/adapter/tools/gates.py, scanner/adapter/store.py, tests | tests first; NodeGoat re-run shows the queue is code-only; code-reviewer + security-reviewer (secrets path) |
