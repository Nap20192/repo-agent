...
| 43 | migrate PipelineV2 to the ADK 2.9 Workflow graph (FunctionNode pre-pass, single_turn stages with output_schema, JoinNode reconciler, dynamic investigator node with parallel workers, terminal reporter); ADR-0001 revisited | architect agent → dev | merged (steps 1–7: c0170a9 0070db3 7388619 d7d6d36 e08697c 31aa995; cutover 673576e; docs) | scanner/app/pipeline_v2.py, graph.py, runner.py, web/fullscan/agent.py, docs/adr/0007 | blueprint reviewed at GATE 1; all pipeline tests green on the Workflow root; adk web renders the graph |
| 44 | Shannon-style discovery: Planner (coverage guarantee, adversarial share), Triage sweep → specialist audit, ported hunting checklists, existence-check gate for synthetic anchors | dev-team | merged (f8c317c..38f0ef4, review approve) | scanner/app/instructions.py, reconcile.py, pipeline_v2.py, tools/gates.py, skills | NodeGoat re-run recalls ≥5 of the known code vulns |
| 45 | Shannon-graph (docs/plans/shannon-graph.md): static Workflow of 23 nodes with route maps, JoinNode, output_schema stages, triage by file batches with coverage artifact, recon sinks inventory, verdict ladder review → viability → confirm (RunStore.annotate), calibrate + export nodes; ADR-0008 | 4 parallel agents (wave 1: core/annotate/settings/instructions/agents; recon; triage+fold; verdict nodes) → 1 agent (wave 2: graph assembly + cutover) | in progress | scanner/core/{workflow,types,ports,settings}.py, scanner/adapter/{store,recon}.py, scanner/app/{instructions,agents,graph_nodes,pipeline,runner}.py, tests | each wave: pytest green + ruff; wave 2: ollama smoke + adk web renders 24 nodes; NodeGoat run on flash-lite when the user agrees |
| 46 | one `adk create` app per agent under web/ (19: the 11 named factories + 8 specialists; folder `secrets_agent` so stdlib `secrets` is not shadowed): lazy `root_agent` → `runner.standalone(name, BUGFINDER_TARGET)`; all apps of one `adk web` process share one wired run per target (Store run, Index, pre-pass anchors); ADK eval per app: seeded evalset (the graph's own payloads on samples/02-vulnshop) + test_config.json with deterministic code graders (eval/adk_metrics.py: read_before_report, verdict_via_gate, schema_*) | claude (this session) | review | scanner/app/runner.py (ROSTER, EXTRA_AGENTS, standalone, _shared_run), web/*/{__init__,agent}.py + *.evalset.json + test_config.json, eval/adk_metrics.py, tests/test_web_agents.py, tests/test_adk_eval.py, pyproject (dev: aiplatform 2.x, pandas, tabulate, rouge-score) | tests first; pytest + ruff green; code-reviewer; `adk web web` lists 19 + fullscan; `adk eval web/verify web/verify/verify.evalset.json` loads the custom metrics |
| 47 | feature folders (ADR-0009): one tool per file + registry (`adapter/tools/<group>/`), one scanner per file (`adapter/scanners/`), one folder per agent with one AgentSpec (`app/agents/<name>/`), one node per file + stage/worker wrappers + edges (`app/graph/`), runner builds every agent from `registry.AGENTS`; behaviour unchanged (edge list byte-for-byte, prompts lifted verbatim) | claude (this session) | review | scanner/adapter/{tools,scanners}/, scanner/app/{agents,graph}/, runner.py, eval/agents.py, tests, docs/adr/0009, CLAUDE.md, README.md, docs/architecture.md | baseline suite unchanged after every phase (Windows LSP/symlink set aside); ruff clean; test_layers + test_contracts template guards; code-reviewer |
| 48 | `fullscan` in adk web takes the target from the message: a directory or https://github.com/<owner>/<repo> (shallow clone into .targets/, `adapter/git.clone`, only that URL shape, `--`, no prompts); root FunctionNode `runner.target_node` → `resolve_target` → `prepare` → the graph nested (`ctx.run_node`) → `finish_run` (SARIF + summary, shared with `scan_full`); a bad message or a failed run answers with `{error}` (redacted) and starts nothing / marks the run failed; local paths confined to WORKSPACE_ROOT (security review HIGH), clone atomic + symlink-safe | claude (this session) | review | scanner/adapter/git.py, scanner/app/target.py, scanner/app/runner.py (finish_run, target_node), web/fullscan/agent.py, tests/test_target.py, tests/test_scan_node.py, tests/test_web_entry.py, tests/fakes.py (_run_node message) | tests first; pytest + ruff green; adk web: bogus path → error answer, no scan; security-reviewer (subprocess + paths) |
| 49 | consultants as sub-agents: `knowledge` and the new `domain` agent (domain_map + grep/read_file/lsp, answers one question with `domain:` refs) are AgentTools of verify/critic/review (both), authz/authz_critic (domain), dependency/dependency_critic (knowledge); `AgentSpec.consults`; function tools consult_domain/consult_knowledge removed (consult_domain's map part → tool `domain_map`); prompts say knowledge(request)/domain(request); DOMAIN_MAX_MODEL_CALLS | claude (this session) | review | scanner/app/agents/domain/, base.py, registry.py, 7 agent folders, scanner/adapter/tools/{consult/domain.py,registry.py}, runner.py, core/settings.py, web/domain/, eval/agents.py, tests | contracts/specialists/wiring tests green; a live run shows domain(...) calls in authz activations |
| 50 | adk web run reliability: every scanner/shell/git subprocess decodes UTF-8 with replacement (Windows cp1252 killed semgrep's reader thread on a 0x81 byte → stdout None); the pre-pass (scan node) and `prepare` run in a worker thread so the event loop / SSE stay alive (the 88 s silence made the dev UI reconnect and ADK cancel the root node) | claude (this session) | review | scanner/adapter/scanners/{process,osv}.py, scanner/adapter/tools/code/shell.py, scanner/adapter/git.py, scanner/app/graph/nodes/scan.py, scanner/app/runner.py, tests/test_subprocess_utf8.py, tests/test_graph_nodes.py | tests green; adk web: /list-apps answers during a scan; semgrep no longer fails on bakery |
| 51 | the small graph (ADR-0010): 24 nodes → 7 (scan, build_skeleton, direct_findings, model, plan, audit, critique, export); 20 agents → 5 (model = architect+threat modeler in one; verify; critic; consultants knowledge, domain); specialists → class overlays (`registry.overlay`); triage sweep, verdict ladder, domain map stage, recon, dominance, notes, web search, compaction and 16 knobs removed; docs/plans + docs/audits + workflow-nodes.md archived | claude (this session) | review | scanner/app/{agents,graph}/, adapter/tools, core/{settings,workflow}.py, runner.py, web/, eval/, tests/, docs/adr/0010, README, CLAUDE.md | suite green; a live run on samples/02-vulnshop confirms CWE-89 + CWE-78 (owner) |
| 52 | fewer entities (ADR-0011): the `knowledge` and `domain` consultants, the enrichment stack (`adapter/knowledge.py`, `tools/knowledge/`, 5 knobs), calibration + `intent`, `viability`/`repro_status`, the adversarial sweep + file baselines, the five-way Index split and the AgentTool wiring (`consults`, `extra_tools`, `per_invocation`, `folder`) removed; osv.dev is one tool `osv_query` (`adapter/osv.py`) on verify/critic; authz evidence `domain:<entity>` is read by the agent itself; 3 agents, 3 web apps | claude (this session) | review | scanner/adapter/{osv,fs,store}.py, adapter/tools/consult/osv_query.py, app/agents/, app/graph/{planning,reconcile}.py, nodes/{export,direct_findings,plan}.py, runner.py, core/{types,ports,settings,rules}.py, web/, tests/, docs/adr/0011 | suite green on the Windows baseline (207 passed; the 18 known LSP/symlink/env failures); ruff clean |
